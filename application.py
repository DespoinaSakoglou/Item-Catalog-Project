...
# @app.route will call the function that follows it whenever the webserver
# receives a request with the url that matches its arg
...
def editItem(category_name, item_name):
    ...
    if editedItem.user_id != login_session['user_id']:
        flash("You are not authorized to edit items you have not created. "
	      "Please add your own items in order to edit items.")
        return redirect(url_for('showCategories'))
    if request.method == 'POST':
...
def deleteItem(category_name, item_name):
    ...
    if itemToDelete.user_id != login_session['user_id']:
        flash("You are not authorized to delete items you have not created. "
	      "Please add your own items in order to delete items.")
        return redirect(url_for('showCategories'))
    if request.method == 'POST':
...
# if statement ensures server only runs if script is executed 
...
if __name__ == '__main__':
    ...
    app.debug = True
    # run the local server with our app, made publicly 
    # available by listening on all public IPs
